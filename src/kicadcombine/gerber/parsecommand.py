...
def read_command(str):
    ...
    if str[:2]=='AD':
        ...
        return ApertureDefinition(pp['ident'], pp['template_name'], template_params)
    ...
    if str[:2]=='AM':
        ...
        return ApertureMacro(macro_name, macro_body)
    ...
    if str[0]=='D':
        ...
        mm=re.match("^(D[1-9][0-9]+)\*$", str)
        if not mm:
            raise Exception("expected D command, length >2, format D{aperture_number >= 10}*")
        ident=mm.groups()[0]
        if not int(ident[1:])>=10:
            raise Exception("expected D command, length >2, format D{aperture_number >= 10}*")
        return SetCurrentAperture(ident)
    ...
    if str[0]=='G':
        ...
        if command in ('G01','G02','G03','G75'):
            return PlotStateCommand(command)
        elif command in ('G36','G37'):
            return RegionStateCommand(command)
        else:
    ...
    if str[:2] == 'LP':
        ...
        return LoadPolarity(mm.groups()[0])
    ...
    if str[:2]=='TD':
        ...
        return DeleteAttributes(n)
    
    
    if False:
        #AB, SR, LM, LR, and LS not used by kicad?
        ...
        if str[:2] == 'LM':
            ...
            return LoadMirroring(mm.groups()[0])
        ...
        if str[:2] == 'LR':
            ...
            return LoadRotation(get_num(mm.groups()[0]))
        ...
        if str[:2] == 'LS':
            ...
            return LoadScaling(get_num(mm.groups()[0]))
